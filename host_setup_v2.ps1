<#
.SYNOPSIS
    SOC Lab - Win 11 Host Splunk Enterprise Setup (final, parser-safe).

.DESCRIPTION
    Runs on the Win 11 host where Splunk Enterprise is installed.
    Enables receiving on port 9997, opens the firewall, verifies the
    listener, tests local connectivity, lists incoming VM connections,
    and prints the SPL queries to verify logs.
...
if (-not $splunkHome) {
    Say "Splunk Enterprise not found." "ERROR"
    Write-Host ""
    Write-Host "Install Splunk Enterprise first: https://www.splunk.com/en_us/download/splunk-enterprise.html" -ForegroundColor Yellow
    if (-not $NoPrompt) { Read-Host "Press Enter to exit" }
    try { Stop-Transcript | Out-Null } catch { }
    exit 1
}
...
# Safety net: ensure stanza exists
...
Write-Host ""
Write-Host "Then run these searches:" -ForegroundColor Green
