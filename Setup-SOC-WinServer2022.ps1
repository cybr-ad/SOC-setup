...
.DESCRIPTION
    ...
    - Ensures hostname is DC01-CORP (renames + reboots + auto-resumes via scheduled task).
    ...
    - Full transcript log to <LabRoot>\Logs\.
    - Safe to re-run at any time.

.PARAMETER SplunkServerIP
    Splunk Enterprise host (Win 11 VMnet8). Default 192.168.10.1.
.PARAMETER SplunkPort
    Splunk receiving port. Default 9997.
.PARAMETER ServerIP
    Static IP for this Windows Server. Default 192.168.10.20.
.PARAMETER PrefixLength
    Subnet prefix length. Default 24.
.PARAMETER Gateway
    Default gateway (VMware NAT router). Default 192.168.10.2.
.PARAMETER InterfaceAlias
    Network adapter name. Default Ethernet0.
.PARAMETER LabRoot
    Working folder inside the VM. Default C:\SOC-Lab.
.PARAMETER TargetHostname
    Computer name to enforce. Default DC01-CORP.
.PARAMETER SkipNetworkConfig
    Do not touch IP/DNS.
.PARAMETER SkipRename
    Do not rename computer.
.PARAMETER SkipAuditing
    Do not configure auditpol / logging.
#>
...
elseif ($currentHost -ieq $TargetHostname) {
    ...
    # Preserve a copy of this script in LabRoot so the resume task is path-safe
    ...
    # Build resume argument list safely (array joined with spaces)
    $argList = @(
        "-NoProfile",
        "-NoLogo",
    ...
    try {
        ...
        Say "Aborting rename to avoid a stuck state. Fix the task scheduler and re-run." "ERROR"
...
if (-not (Test-Path "$LabRoot\splunkforwarder.msi")) {
    ...
    Say "Copy splunkforwarder.msi into $LabRoot or into the class file\bin folder." "ERROR"
    try { Stop-Transcript | Out-Null } catch { }
    exit 1
}
Say "MSI present." "OK"
...
# inputs.conf -- always write explicit host = TargetHostname
$inputsLines = @(
    "[default]",
    "host = $TargetHostname",
    "",
...
if ($tcpOK) {
    ...
    Say "Check Win 11 firewall: New-NetFirewallRule -DisplayName 'Splunk 9997' -Direction Inbound -Protocol TCP -LocalPort 9997 -Action Allow" "WARN"
    Say "Check Win 11 Splunk: & 'C:\Program Files\Splunk\bin\splunk.exe' enable listen 9997 -auth socadmin:P@ssw0rd2026!" "WARN"
}
